...
df_sport_counts = df_clustered.groupby(['Cluster', 'Sport_Name']).size().reset_index(name='Count')
df_sport_counts = df_sport_counts.dropna(subset=['Sport_Name'])  # Drop rows with NaN sport
...
pivot = pivot[pivot.sum(axis=1) >= 3]  # Only include sports with at least 3 participants
...
centers_melted = pd.melt(centers, id_vars=['Cluster'], var_name='Component', value_name='Value')
